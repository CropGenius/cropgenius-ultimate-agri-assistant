...
const cropDiseases = {
  tomato: [
    ...
    },
    {
      ...
      severity: 'medium',
      treatments: ['neem oil', 'crop rotation', 'mulching'],
    },
    {
  ...
  maize: [
    ...
    },
    {
      ...
      severity: 'low',
      treatments: ['fungicide sprays', 'resistant varieties', 'early planting'],
    },
  ],
  coffee: [
    {
    ...
    },
    {
      ...
      scientific: 'Colletotrichum kahawae',
      severity: 'high',
      treatments: ['copper-based fungicides', 'pruning', 'plant spacing'],
    },
    {
      name: 'Brown Eye Spot',
      scientific: 'Cercospora coffeicola',
      severity: 'medium',
      treatments: ['balanced fertilization', 'fungicides', 'shade management'],
    },
  ],
  banana: [
    {
    ...
    },
    {
      ...
      severity: 'high',
      treatments: ['fungicide application', 'leaf pruning', 'proper spacing'],
    },
    {
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Processing crop scan for user ${userId || 'anonymous'}, crop type: ${cropType}`
    );
    ...
    const diseases =
      cropDiseases[detectedCrop as keyof typeof cropDiseases] ||
      cropDiseases.tomato;
    ...
    const selectDisease = () => {
      ...
      diseases.forEach((disease) => {
        totalWeight +=
          severityWeights[disease.severity as keyof typeof severityWeights];
      });
    ...
    const country =
      location?.country ||
      ['kenya', 'nigeria', 'tanzania', 'uganda', 'ethiopia'][
        Math.floor(Math.random() * 5)
      ];
    const availableRegions =
      regions[country as keyof typeof regions] || regions.kenya;
    const region =
    ...
    const getLocalStores = () => {
      const storeNames = [
        ...
        'Kilimo Stores',
        "Farmers' Market",
        'Green Harvest Supplies',
        'Rural AgriTech',
        'Farm Solutions',
        'EcoFarm Products',
        'Organic Growth Center',
      ];
    ...
    const estimatedYieldImpact = Math.floor(
      affectedArea *
        severityMultiplier[
          selectedDisease.severity as keyof typeof severityMultiplier
        ]
    );
    ...
    const scanResult = {
      ...
      affectedArea,
      recommendedTreatments: selectedDisease.treatments.map(
        (treatment) =>
          treatment.charAt(0).toUpperCase() +
          treatment.slice(1) +
          (Math.random() > 0.5 ? ' within 48 hours' : '')
      ),
      preventiveMeasures: [
    ...
    console.log(
      `Scan result generated: ${scanResult.diseaseDetected} with ${scanResult.confidenceLevel}% confidence`
    );
