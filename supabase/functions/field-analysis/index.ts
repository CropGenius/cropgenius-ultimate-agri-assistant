...
// Generate insights based on available data
function generateInsights(field: any, weatherData: any, nearbyCrops: any[]) {
  const insights = [];
  ...
  if (field.soil_type) {
    ...
    insights.push(
      `Your soil type (${soilType}) is generally suitable for ${crops}.`
    );
    ...
    if (soilProps) {
      if (soilProps.ph_level) {
        ...
        if (ph < 5.5) {
          insights.push(
            `Soil pH is acidic (${ph}). Consider lime application to raise pH for better nutrient availability.`
          );
        } else if (ph > 7.5) {
          insights.push(
            `Soil pH is alkaline (${ph}). Some crops may face micronutrient deficiencies.`
          );
        } else {
  ...
  if (weatherData) {
    ...
    if (temp > 30) {
      insights.push(
        `High temperatures detected (${temp}°C). Consider drought-resistant varieties and increased irrigation.`
      );
    }

    if (humidity > 80 && temp > 25) {
      insights.push(
        `High humidity and temperature conditions increase risk of fungal diseases. Monitor crops regularly.`
      );
    }

    if (precip < 2 && field.irrigation_type === 'rainfed') {
      insights.push(
        `Low rainfall detected. Rainfed fields may need supplemental irrigation in coming days.`
      );
    }
  }
  ...
  if (nearbyCrops && nearbyCrops.length > 0) {
    const popularCrops = nearbyCrops
      .map((c) => c.crop_name)
      .slice(0, 3)
      .join(', ');
    insights.push(
      `Nearby farmers are successfully growing ${popularCrops} in similar conditions.`
    );
  }
  ...
  if (insights.length < 3) {
    insights.push(
      'Regular soil testing and field monitoring will help optimize your farming decisions.'
    );
  }
