...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Checking AI insights for ${userId ? 'user ' + userId : 'all users'}`
    );

    // Query to get users to check
    let usersQuery = supabase
      .from('user_memory')
      .select('user_id, memory_data');
    ...
    for (const user of users) {
      ...
      if (hoursSinceLastInsight < 12) {
        console.log(
          `Skipping user ${user.user_id} - insights checked ${hoursSinceLastInsight.toFixed(1)} hours ago`
        );
        continue;
      }
      ...
      if (fieldsError) {
        console.error(
          `Error fetching fields for user ${user.user_id}:`,
          fieldsError
        );
        continue;
      }
      ...
      if (insertError) {
        console.error(
          `Error storing insight for user ${user.user_id}:`,
          insertError
        );
        continue;
      }
      ...
      if (updateError) {
        console.error(
          `Error updating memory for user ${user.user_id}:`,
          updateError
        );
      }
      ...
      if (memory.whatsappOptIn) {
        ...
        try {
          ...
          console.error(
            `Error sending WhatsApp message for user ${user.user_id}:`,
            whatsappError
          );
        }
      }
