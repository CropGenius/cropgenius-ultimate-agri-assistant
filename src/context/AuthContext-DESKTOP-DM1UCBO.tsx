...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    if (import.meta.env.DEV) {
      console.log(
        '[DEV] Authentication bypassed for development. Using mock user and farmId:',
        DEV_FARM_ID
      );
    ...
    console.log(
      'AuthProvider: Setting up Supabase auth listener for production.'
    );
    ...
    const {
      ...
      console.log(
        ...
        session?.user?.id ? `User: ${session.user.id}` : 'No user'
      );
      setAuthState((prev) => ({
        ...prev,
    ...
    return () => {
      console.log(
        'Cleaning up Supabase auth subscription and refresh interval.'
      );
  ...
  const checkUserFarm = async (userId: string) => {
    if (import.meta.env.DEV) {
      console.log(
        `[DEV] Mock farm check for user: ${userId}. Using DEV_FARM_ID: ${DEV_FARM_ID}`
      );
    ...
    try {
      ...
      if (error) {
        ...
        setAuthState((prev) => ({
          ...prev,
          farmId: null,
          error: `Farm fetch error: ${error.message}`,
        }));
        return;
      }
      ...
      setAuthState((prev) => ({
        ...prev,
        farmId: null,
        error: `Farm fetch critical error: ${e.message}`,
      }));
    }
  };
