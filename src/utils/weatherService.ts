  ...
  try {
    ...
    if (rainForecast) {
      ...
      const hoursDiff = Math.round(
        (rainTime.getTime() - currentTime.getTime()) / (1000 * 60 * 60)
      );
    ...
    const rainForecasts = next24hForecasts.filter(
      (item: any) =>
        item.weather[0].main === 'Rain' || item.weather[0].main === 'Drizzle'
    );
    const rainChance = Math.round(
      (rainForecasts.length / next24hForecasts.length) * 100
    );
...
export const getFarmingRecommendations = async (
  weather: WeatherData,
  crops: string[]
): Promise<string[]> => {
  try {
    ...
    if (weather.soilMoisture < 40) {
      recommendations.push(
        'Irrigation recommended - soil moisture levels are below optimal thresholds for most crops'
      );
    }

    if (weather.rainChance > 70) {
      recommendations.push(
        'Delay pesticide application - high chance of rain in the next 24 hours may wash away chemicals'
      );
    }

    if (weather.temp > 32) {
      recommendations.push(
        'High temperature alert - consider providing shade for sensitive crops and additional irrigation'
      );
    }

    if (weather.windSpeed > 20) {
      recommendations.push(
        'High wind alert - secure farm structures and delay spraying operations'
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        'Conditions are currently optimal for most farming operations'
      );
    }
