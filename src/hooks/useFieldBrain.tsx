...
// Provider component
export const FieldBrainProvider = ({
  children,
  userId,
}: {
  children: ReactNode;
  ...
  // Get field insights
  const getFieldInsights = useCallback(
    (fieldId?: string) => {
      if (!isInitialized) return [];
  ...
  const getSuggestedAction = useCallback(() => {
    ...
    try {
      ...
      return {
        action: 'Unable to suggest actions right now',
  ...
  const getFieldHealth = useCallback(() => {
    ...
    try {
      ...
      return {
        score: 0,
        assessment: 'Unable to assess field health right now',
      };
    }
  }, [isInitialized]);
