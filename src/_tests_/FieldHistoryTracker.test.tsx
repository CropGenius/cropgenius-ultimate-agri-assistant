              ...
              {item.details && (
                <div data-testid={`history-details-${item.id}`}>
                  {Object.entries(item.details).map(([key, value]) => (
                    <div
                      key={key}
                      data-testid={`history-detail-${item.id}-${key}`}
                    >
                      {key}: {String(value)}
                    </div>
                  ))}
...
describe('FieldHistoryTracker', () => {
  ...
  it('shows field history log with all entries', () => {
    render(
      <FieldHistoryTracker history={mockHistory} showFullHistory={true} />
    );
    ...
    mockHistory.forEach((item) => {
      ...
      expect(screen.getByTestId(`history-action-${item.id}`)).toHaveTextContent(
        item.action
      );
      ...
      if (item.details) {
        Object.entries(item.details).forEach(([key, value]) => {
          const detailElement = screen.getByTestId(
            `history-detail-${item.id}-${key}`
          );
