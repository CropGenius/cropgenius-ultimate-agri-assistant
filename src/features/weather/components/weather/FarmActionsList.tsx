...
export default function FarmActionsList({
  location,
  crops,
}: FarmActionsListProps) {
  ...
  const generateActions = () => {
    const allPossibleActions = [
      {
        ...
        description:
          "Rain expected in 36 hours. Skip today's irrigation to save water and resources.",
        urgency: 'medium',
        icon: 'droplet',
        expiresIn: '36 hours',
        efficiencyGain: 12,
        ...
        actionButton: {
          ...
          onClick: () =>
            handleAutomatedAction(
              'irrigation',
              'Adjusted irrigation schedule automatically based on weather forecast. Water resources saved.'
            ),
        },
        chartData: {
          type: 'rainfall',
      ...
      },
      {
        ...
        title: 'Protect Crops',
        description:
          'Heatwave coming this weekend. Increase soil cover and prepare shade cloth for sensitive crops.',
        urgency: 'high',
        icon: 'shield-alert',
        expiresIn: '2 days',
        efficiencyGain: 19,
        ...
        actionButton: {
          ...
          onClick: () =>
            handleAutomatedAction(
              'heatwave',
              'AI has generated a heat protection plan for your crops. Instructions sent to your mobile device.'
            ),
        },
        chartData: {
          type: 'temperature',
      ...
      },
      {
        ...
        title: 'Harvest Soon',
        description:
          'Rain may spoil stored maize if not harvested before Friday. Schedule harvest team now.',
        urgency: 'high',
        icon: 'tractor',
        expiresIn: '3 days',
        efficiencyGain: 22,
        ...
        actionButton: {
          ...
          onClick: () =>
            handleAutomatedAction(
              'harvest',
              'AI has scheduled your harvest team for Thursday. Text messages sent to all workers.'
            ),
        },
        chartData: {
          type: 'harvest',
      ...
      },
      {
        ...
        description:
          'Market expects maize shortage due to regional flooding. Consider selling stored crop next week.',
        urgency: 'low',
        ...
        actionButton: {
          ...
          onClick: () =>
            handleAutomatedAction(
              'market',
              'AI is connecting you with the top 3 buyers in your region. Expected price: 12% above market average.'
            ),
        },
        chartData: {
          type: 'price',
      ...
      },
      {
        ...
        description:
          'Current humidity levels create high risk for fungal diseases. Inspect crops this week.',
        urgency: 'medium',
        icon: 'x-circle',
        expiresIn: '5 days',
        efficiencyGain: 17,
        ...
        actionButton: {
          ...
          onClick: () =>
            handleAutomatedAction(
              'pests',
              'AI has analyzed your crops and generated a precise organic treatment plan to prevent fungal diseases.'
            ),
        },
        chartData: {
          type: 'humidity',
    ...
    const highUrgencyActions = allPossibleActions.filter(
      (a) => a.urgency === 'high'
    );
    const mediumUrgencyActions = allPossibleActions.filter(
      (a) => a.urgency === 'medium'
    );
    const lowUrgencyActions = allPossibleActions.filter(
      (a) => a.urgency === 'low'
    );
    ...
    if (highUrgencyActions.length > 0) {
      selectedActions.push(
        highUrgencyActions[
          Math.floor(Math.random() * highUrgencyActions.length)
        ]
      );
    }
    ...
    const avgEfficiency =
      selectedActions.reduce((sum, action) => sum + action.efficiencyGain, 0) /
      totalActions;
    const totalYieldImpact = selectedActions.reduce(
      (sum, action) => sum + action.yieldImpact,
      0
    );
  ...
  const toggleActionComplete = (id: string) => {
    ...
    if (completedActions.includes(id)) {
      newCompletedActions = completedActions.filter(
    ...
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center text-muted-foreground">
        ...
        <p className="text-sm mt-1">
          Weather conditions are optimal for your crops
        </p>
      </div>
    );
  }
