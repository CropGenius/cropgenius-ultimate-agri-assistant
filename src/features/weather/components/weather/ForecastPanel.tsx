...
export default function ForecastPanel({ location }: ForecastPanelProps) {
  ...
  const generateForecast = () => {
    ...
    for (let i = 0; i < days; i++) {
      ...
      for (let h = 0; h < 24; h += 3) {
        ...
        const hourRainProb = Math.max(
          0,
          Math.min(100, rainProb + (Math.random() - 0.5) * 20)
        );
  ...
  const getFarmAction = (conditionIndex: number, rainProb: number) => {
    if (conditionIndex >= 4) {
      return {
        ...
        description:
          'Heavy rain/storm expected. Secure structures and improve drainage.',
        urgency: 'high',
      };
      ...
      return {
        ...
        description:
          'Light rain expected. Wait until after rainfall to apply fertilizer.',
        urgency: 'medium',
      };
      ...
      return {
        ...
        description:
          'Dry conditions expected. Schedule irrigation for optimal moisture.',
        urgency: 'medium',
      };
    } else {
      return {
        ...
        description:
          'Weather conditions suitable for standard farming activities.',
        urgency: 'low',
      };
    }
  };
  ...
  const getWeatherIcon = (icon: string, size = 5) => {
    switch (icon) {
      ...
      case 'cloud':
        return <Cloud className={`h-${size} w-${size} text-slate-400`} />;
      case 'cloud-rain':
        return <CloudRain className={`h-${size} w-${size} text-blue-500`} />;
      case 'cloud-drizzle':
        return <CloudDrizzle className={`h-${size} w-${size} text-blue-400`} />;
      case 'cloud-lightning':
        return (
          <CloudLightning className={`h-${size} w-${size} text-purple-500`} />
        );
      case 'cloud-snow':
        return <CloudSnow className={`h-${size} w-${size} text-blue-300`} />;
      default:
        return <Sun className={`h-${size} w-${size} text-amber-500`} />;
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1">
            <div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {Math.round(selectedDay.temp.max)}° /{' '}
                    {Math.round(selectedDay.temp.min)}°
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {selectedDay.windSpeed.toFixed(1)} km/h
                  </span>
                </div>
                <div className="flex justify-between">
              ...
              <div
                className={`mt-6 p-3 rounded-lg ${getUrgencyColor(selectedDay.farmAction.urgency)}`}
              >
                ...
                <p className="text-sm mt-1">
                  {selectedDay.farmAction.description}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
              {selectedDay.hourly.map((hour: any, index: number) => (
                <div
                  key={index}
                  className="bg-slate-50 dark:bg-slate-900 rounded-lg p-2 text-center"
                >
