...
const verifyFarmOwnership = async (
  ...
  try {
    ...
    if (!userId) {
      console.warn(
        '⚠️ [verifyFarmOwnership] Missing user ID, proceeding anyway'
      );
      return true;
    }
    ...
    try {
      ...
      if (error) {
        ...
        const newFarmId = await verifyOrCreateFarm(supabase, userId);
        console.log(
          '✅ Created fallback farm due to verification error:',
          newFarmId
        );
        return true;
      }
      ...
      if (data && data.user_id !== userId) {
        console.warn(
          `⚠️ User ${userId} attempted to access farm ${farmId} owned by ${data.user_id}`
        );
        ...
        try {
          ...
          if (userFarm?.id) {
            ...
            console.log(
              `⚠️ Redirecting user to their own farm ${userFarm.id} instead of ${farmId}`
            );
      ...
      const newFarmId = await verifyOrCreateFarm(supabase, userId);
      console.log(
        '✅ Created emergency farm due to verification error:',
        newFarmId
      );
    }
      ...
      if (error) {
        ...
        if (minimalError) {
          ...
          toast.warning('Saved locally', {
            description:
              'Your field has been saved offline and will sync when you reconnect.',
          });
...
export const getAllFields = async (
  ...
  try {
    ...
    const offlineFields = getOfflineData<Field>(OFFLINE_FIELDS_KEY).filter(
      (field) => field.user_id === userId && !field.is_synced
    );
  ...
  if (isOnline()) {
    try {
      ...
      const index = offlineFields.findIndex(
        (f) => f.id === field.id || f.offline_id === field.offline_id
      );
      ...
      toast.warning('Saved offline', {
        description:
          'Your changes have been saved offline and will sync when you reconnect.',
      });
...
export const deleteField = async (
  ...
  if (isOnline()) {
    ...
    const fieldIndex = offlineFields.findIndex(
      (f) => f.id === fieldId || f.offline_id === fieldId
    );
    ...
    if (fieldIndex >= 0) {
      ...
      if (
        ...
        !offlineFields[fieldIndex].is_synced
      ) {
    ...
    toast.info('Marked for deletion', {
      description:
        'This field will be deleted when you reconnect to the internet.',
    });
...
export const syncOfflineData = async (
  ...
  try {
    // Sync fields
    const offlineFields = getOfflineData<FieldWithDeleteFlag>(
      OFFLINE_FIELDS_KEY
    ...
    for (const field of offlineFields) {
      if (field.deleted) {
        ...
        if (!error && data) {
          ...
          const allOfflineFields =
            getOfflineData<FieldWithDeleteFlag>(OFFLINE_FIELDS_KEY);
          const index = allOfflineFields.findIndex(
    ...
    saveOfflineData(
      ...
      allOfflineFields
        .filter((f) => !f.deleted)
        .map((f) => ({ ...f, is_synced: true }))
    );

    toast.success('Data synchronized', {
      description:
        'Your field data has been successfully synchronized with the server.',
    });
    ...
    toast.error('Sync failed', {
      description:
        'There was a problem synchronizing your data. Please try again.',
    });
...
export const getFieldById = async (
  ...
  try {
    ...
    const offlineFields = getOfflineData<Field>(OFFLINE_FIELDS_KEY);
    const offlineField = offlineFields.find(
      (f) => f.id === fieldId || f.offline_id === fieldId
    );
