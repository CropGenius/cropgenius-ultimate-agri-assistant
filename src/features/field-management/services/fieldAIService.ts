...
export const analyzeField = async (
  fieldId: string
): Promise<FieldAnalysisResult | null> => {
  try {
    ...
    if (!aiError && aiData) {
      ...
      return {
        fieldId: fieldId,
        insights: [
          ...aiData.crop_rotation.suggestions.map(
            (crop: string) => `Consider planting ${crop} next.`
          ),
          ...aiData.soil_health.recommendations,
          ...aiData.disease_risks.risks.map((risk: any) =>
            risk.risk > 0.5
              ? `Warning: ${risk.disease} risk detected.`
              : `Low risk of ${risk.disease}.`
          ),
        ],
 ...
 */
export const checkFieldRisks = async (
  fieldId: string
): Promise<{
  hasRisks: boolean;
  risks: {
    ...
    const simulatedRisks = [
      {
        ...
        description:
          'Current humidity and temperature conditions favor this pest. Monitor fields every 3-5 days.',
      },
      {
        name: 'Fungal Blight',
        likelihood: 'low' as const,
        description:
          'Low risk due to current dry conditions, but watch for sudden weather changes.',
      },
      {
        name: 'Stem Borer',
        likelihood: 'high' as const,
        description:
          'High temperatures and nearby infestations indicate elevated risk. Consider preventative treatments.',
      },
    ];
