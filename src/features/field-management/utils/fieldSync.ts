...
export const syncOfflineFields = async (): Promise<{
  ...
  console.log(
    `🔄 [syncOfflineFields] Attempting to sync ${offlineFields.length} offline fields`
  );
  ...
  for (const field of offlineFields) {
    try {
      ...
      const fieldCrops = offlineCrops.filter(
        (crop: any) => crop.field_id === field.id
      );
      ...
      for (const crop of fieldCrops) {
        try {
          ...
          const updatedOfflineCrops = offlineCrops.filter(
            (c: any) => c.id !== crop.id
          );
          localStorage.setItem(
            'cropgenius_offline_crops',
            JSON.stringify(updatedOfflineCrops)
          );
        } catch (cropError) {
          console.error(
            '❌ [syncOfflineFields] Failed to sync crop:',
            cropError
          );
  ...
  // Log results
  console.log(
    `🔄 [syncOfflineFields] Sync complete: ${syncedCount} synced, ${failedCount} failed, ${remainingFields.length} remaining`
  );
