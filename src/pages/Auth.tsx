...
export default function Auth() {
  ...
  useEffect(() => {
    if (user) {
      console.log(
        'Auth page: User already authenticated, redirecting to:',
        from
      );
    ...
    try {
      ...
      setTimeout(() => {
        if (user) {
          ...
          setAuthError(
            'Still unable to authenticate. Please try logging in again.'
          );
        }
        setIsRetrying(false);
      }, 1500);
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <Card className="w-full max-w-md">
        ...
        <CardFooter className="text-xs text-muted-foreground text-center">
          By continuing, you agree to CROPGenius Terms of Service and Privacy
          Policy
        </CardFooter>
      </Card>
    </div>
