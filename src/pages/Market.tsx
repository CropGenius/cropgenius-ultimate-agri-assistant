...
const Market = () => {
  ...
  const [aiInsights, setAiInsights] = useState({
    ...
    marketOverview:
      'Market prices for maize and rice are trending upward. Tomato prices are falling due to increased supply. Best time to sell beans and soybeans.',
    topOpportunities: [
      {
        crop: 'Maize',
  ...
  const listCrop = (cropId: number) => {
      ...
      holdRecommendation: {
        ...
        expectedIncrease:
          crop.marketTrend.status === 'rising'
            ? Math.abs(crop.marketTrend.changePercent)
            : 0,
      },
      seller: {
        name: 'Emmanuel',
    ...
    setMyCrops(
      myCrops.map((c) => (c.id === cropId ? { ...c, isListed: true } : c))
    );
  ...
  const joinBulkDeal = (dealId: number) => {
    ...
    setBulkDeals(
      bulkDeals.map((d) =>
        d.id === dealId
          ? { ...d, farmersJoined: d.farmersJoined + 1, isNew: false }
          : d
      )
    );
  ...
  const handlePostTrade = async (e: FormEvent) => {
    e.preventDefault();
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">
                AI-powered trading platform for maximum profits
              </p>
            </div>
            <Button
          ...
          <Card className="mt-4 border-crop-green-200 bg-crop-green-50">
            <CardHeader className="pb-2">
              <div className="flex justify-between items-center">
                <CardTitle className="text-lg text-crop-green-700">
                  ...
                  >
                    ...
                    <div>
                      <h4 className="font-medium text-gray-800">
                        {opportunity.crop}: {opportunity.action}
                      </h4>
          ...
          <TabsContent value="marketplace" className="mt-4">
            <div className="space-y-4">
              ...
              {marketListings.map((listing) => (
                <Card
                  key={listing.id}
                  className={`overflow-hidden ${listing.isMine ? 'border-crop-green-400 bg-crop-green-50' : ''}`}
                >
                        ...
                        <div className="flex flex-col sm:flex-row justify-between sm:items-end">
                          <div>
                            ...
                            <div className="flex items-end">
                              <p className="text-2xl font-bold text-crop-green-700">
                                {listing.price}
                              </p>
                              <p className="text-sm ml-1 mb-1 text-gray-500">
                                ...
                                {Math.round(
                                  (listing.price / listing.marketPrice - 1) *
                                    100
                                )}
                                % vs market)
                              </p>
                            ...
                            <p className="text-xs text-gray-600">
                              Total value: {listing.price * listing.quantity}
                            </p>
                          </div>
                        ...
                        {listing.bulkDeal && (
                          <div className="mt-3 bg-amber-50 p-3 rounded-lg border border-amber-100">
                            ...
                            <p className="text-xs text-amber-700 mt-1">
                              {listing.bulkDeal.farmersJoined}/
                              {listing.bulkDeal.targetFarmers} farmers joined
                              for {listing.bulkDeal.bonusPercentage}% bonus
                              pricing
                            </p>
                            <div className="mt-1 h-1.5 w-full bg-amber-200 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${(listing.bulkDeal.farmersJoined / listing.bulkDeal.targetFarmers) * 100}%`,
                                }}
                              ></div>
                            </div>
                          </div>
                        ...
                        {listing.holdRecommendation.recommend && (
                          <div className="mt-3 bg-crop-green-50 p-3 rounded-lg border border-crop-green-100">
                            ...
                            <p className="text-xs text-crop-green-700 mt-1">
                              Hold for {listing.holdRecommendation.days} more
                              days for a potential{' '}
                              {listing.holdRecommendation.expectedIncrease}%
                              price increase
                            </p>
                          </div>
                        )}
                      ...
                      <div className="flex-grow">
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-3">
                          ...
                          <div>
                            ...
                            <p className="font-medium">
                              {crop.currentPrice} per {crop.unit}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="mb-3">
                          ...
                          <div className="flex items-end">
                            ...
                            <p className="text-sm ml-1 mb-1 text-gray-500">
                              (
                              {crop.suggestedPrice > crop.currentPrice
                                ? '+'
                                : ''}
                              {Math.round(
                                (crop.suggestedPrice / crop.currentPrice - 1) *
                                  100
                              )}
                              % vs market)
                            </p>
                          ...
                          <p className="text-xs text-gray-600">
                            Total value: {crop.suggestedPrice * crop.quantity}
                          </p>
                        </div>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const listing = marketListings.find(
                              (l) => l.cropType === crop.cropType && l.isMine
                            );
                            if (listing) {
