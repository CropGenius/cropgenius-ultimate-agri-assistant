...
// Get all soil types
export const getSoilTypes = async (): Promise<{
  data: SoilType[];
  error: string | null;
}> => {
  try {
...
// Get soil type by ID
export const getSoilTypeById = async (
  id: string
): Promise<{ data: SoilType | null; error: string | null }> => {
  try {
    const { data, error } = await supabase
      .from('soil_types')
      .select('*')
...
export const getSoilTypeByName = async (
  name: string
): Promise<{ data: SoilType | null; error: string | null }> => {
  try {
    const { data, error } = await supabase
