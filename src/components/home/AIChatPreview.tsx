...
export default function AIChatPreview() {
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        role: 'assistant',
        content:
          "Hello farmer! I'm your AI farming assistant. How can I help you today?",
      },
    ]);
    ...
    setTimeout(() => {
      ...
      toast.info('AI Assistant Update', {
        description:
          'Generated personalized recommendations based on your crops',
      });
    }, 3000);
  }, []);
  ...
  const handleSendMessage = (content: string) => {
    ...
    setTimeout(() => {
      ...
      if (
        content.toLowerCase().includes('maize') ||
        ...
        content.toLowerCase().includes('tomato') ||
        content.toLowerCase().includes('blight')
      ) {
        response =
        ...
        content.toLowerCase().includes('fertilizer') ||
        content.toLowerCase().includes('bean')
      ) {
        response =
        ...
        content.toLowerCase().includes('soil') ||
        ...
        content.toLowerCase().includes('coffee')
      ) {
        response =
        ...
        content.toLowerCase().includes('harvest') ||
      ...
      const interval = setInterval(() => {
        if (displayedResponse.length < response.length) {
          displayedResponse = response.substring(
            0,
            displayedResponse.length + 3
          );
