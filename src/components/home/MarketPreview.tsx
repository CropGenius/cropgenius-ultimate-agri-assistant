      ...
      const crops: CropPricing[] = [
        {
          ...
          trend: 'up',
          aiRecommendation:
            'Strong demand due to regional shortages. AI predicts continued price increases over next 10 days.',
          action: 'hold',
          urgency: 'medium',
          prediction: {
            days: 10,
        ...
        },
        {
          ...
          trend: 'down',
          aiRecommendation:
            'Temporary price drop due to market oversupply. Hold until next month for better prices.',
          action: 'hold',
          urgency: 'low',
          prediction: {
            days: 30,
        ...
        },
        {
          ...
          trend: 'up',
          aiRecommendation:
            'URGENT: Current prices exceptionally high due to regional shortages. Sell immediately to maximize profit.',
          action: 'sell',
          urgency: 'high',
          ...
          prediction: {
            ...
            price: 76.45,
            reason:
              'Incoming harvest from northern region will increase supply',
          },
        },
      ];
  ...
  const updateRandomPrice = () => {
    ...
    setMarketData((prevData) => {
      ...
      const newPrice = parseFloat(
        (oldPrice + (oldPrice * changeAmount) / 100).toFixed(2)
      );

      crop.currentPrice = newPrice;
      crop.change = parseFloat(
      ...
      if (Math.abs(crop.change) > 3) {
        if (crop.trend === 'up') {
          crop.aiRecommendation =
            'Prices rising quickly. Consider selling if ready for market.';
          crop.action = 'sell';
          crop.urgency = 'medium';
        } else {
          crop.aiRecommendation =
            'Prices dropping. Hold until market stabilizes.';
          crop.action = 'hold';
          crop.urgency = 'low';
        }
  ...
  const refreshMarketData = () => {
    ...
    setTimeout(() => {
      ...
      setMarketData((prevData) =>
        prevData.map((crop) => {
          const changeAmount =
            (Math.random() * 2 - 1) * (Math.random() > 0.7 ? 3 : 1);
          const oldPrice = crop.currentPrice;
          const newPrice = parseFloat(
            (oldPrice + (oldPrice * changeAmount) / 100).toFixed(2)
          );
                      ...
                      {crop.prediction && (
                        <div className="flex items-center gap-2 mt-2 text-xs">
                          ...
                          <div className="text-amber-800 dark:text-amber-200">
                            ...
                            ${crop.prediction.price.toFixed(2)} in{' '}
                            {crop.prediction.days} days
                          </div>
                        </div>
                      )}
