...
export default function CropScannerPreview() {
    ...
    setTimeout(() => {
      const history: ScanHistory[] = [
        ...
        },
        {
          ...
          confidence: 89,
          recommendations: [
            'Apply organic fungicide within 24 hours',
        ...
        },
        {
          ...
          confidence: 95,
          recommendations: [
            'Immediate application of copper-based fungicide',
  ...
  const getStatusBadge = (status: ScanStatus) => {
    switch (status) {
      ...
      case 'issue':
        return <Badge className="bg-amber-500">Issue Detected</Badge>;
      case 'critical':
                ...
                {scanHistory.map((scan) => (
                  ...
                  >
                    <div className="p-3">
                        ...
                        <div className="flex items-start gap-2">
                          <div className="mt-0.5">
                            {scan.status === 'healthy' ? (
                              ...
                              <AlertTriangle
                                className={`h-5 w-5 text-red-500 ${animatePing && scan.status === 'critical' ? 'animate-ping' : ''}`}
                              />
                            )}
                          </div>
                          <div>
                      ...
                      {scan.status !== 'healthy' && (
                        <div
                          className={`mt-2 p-2 rounded-md text-xs ${getStatusColor(scan.status)}`}
                        >
