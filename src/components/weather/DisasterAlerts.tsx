    ...
    const sampleAlerts = [
      {
        ...
        description: "Heavy rainfall expected in the next 48 hours. Prepare drainage systems and move equipment to higher ground.",
      ...
      },
      {
        ...
        description: "Thunderstorms likely with potential for lightning strikes. Ensure livestock is sheltered and electrical equipment protected.",
      ...
      },
      {
        ...
        severity: "medium",
        title: "Heat Advisory",
        description: "Temperatures expected to rise above normal levels. Increase irrigation and provide shade for sensitive crops.",
        timeframe: "Next week",
        icon: "thermometer",
      },
    ];
    ...
    <Card className="h-full">
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2">
          ...
          {alerts.length > 0 && (
            <Badge variant="outline" className={`ml-2 ${alerts.some(a => a.severity === 'high') ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-100' : 'bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-100'}`}>
      ...
      <CardContent>
        {alerts.length > 0 ? (
          <div className="space-y-4">
            {alerts.map((alert) => (
              <div key={alert.id} className="bg-slate-50 dark:bg-slate-900 rounded-lg p-4 border-l-4 border-red-500">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold flex items-center gap-2">
                      {alert.title}
                      <Badge variant="outline" className={alert.severity === 'high' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'}>
                        {alert.severity === 'high' ? 'Urgent' : 'Warning'}
                      </Badge>
                    </h3>
                    <p className="text-sm mt-1">{alert.description}</p>
                    <div className="mt-2 flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">Expected: {alert.timeframe}</span>
          ...
          <div className="h-full flex flex-col items-center justify-center py-8 text-center text-muted-foreground">
            ...
            <p className="text-sm mt-1">Your farm location currently has no weather threats</p>
          </div>
        )}
