...
const pathsAllowedWithoutFarm: string[] = [
  /* e.g., '/profile', '/settings/account' */
];
...
const ProtectedRoute = ({ children }: ProtectedRouteProps) => {
  ...
  if (isLoading && !isDevPreview) {
    // Don't show loading screen if in dev preview with mock data
    return (
  ...
  if (!user && !isDevPreview) {
    // If not loading, and no user (and not in dev preview mode)
    console.log(
      'ProtectedRoute: No user, redirecting to /auth from',
      location.pathname
    );
  ...
  if (
    ...
    !farmId &&
    !isDevPreview &&
    !pathsAllowedWithoutFarm.includes(location.pathname)
  ) {
    console.log(
      'ProtectedRoute: User authenticated, no farmId, showing FarmOnboarding for',
      location.pathname
    );
    return (
