...
const GeniusBadge = ({ type, showShareOptions = true }: GeniusBadgeProps) => {
  ...
  const copyShareLink = () => {
    navigator.clipboard.writeText(
      `${window.location.origin}?ref=badge&type=${type}`
    );
  ...
  return (
    <Card
      className={`${badgeData.color} border-none shadow-sm overflow-hidden`}
    >
      <div
        className={`absolute top-0 right-0 w-24 h-24 -mr-8 -mt-8 rounded-full ${badgeData.badgeColor} opacity-10`}
      />

      <CardHeader>
        <div className="flex items-center gap-3">
          <div
            className={`p-2 rounded-full ${badgeData.badgeColor} text-white`}
          >
            {badgeData.icon}
          </div>
          <div>
            <CardTitle className={badgeData.textColor}>
              {badgeData.title}
            </CardTitle>
          ...
          <Dialog open={isShareOpen} onOpenChange={setIsShareOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Share your farming success and invite friends to join
                  CROPGenius!
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="grid gap-4 py-4">
                <p className="text-sm border-l-4 border-green-500 pl-3 py-2 bg-green-50 dark:bg-green-900/20 rounded">
                  "I just earned the {badgeData.title} badge on CROPGenius! 🌱
                  I'm using AI to optimize my farm. Join me and get Pro features
                  for free!"
                </p>
