...
const FieldDashboard: FC<FieldDashboardProps> = ({
  ...
  const fetchFields = useCallback(async () => {
    if (!user || !farmId) {
      if (!authLoading) {
        // Only show toast if auth is not actively loading
        toast.error('Authentication Error', {
          description: 'User or farm context is missing. Cannot fetch fields.',
        });
      }
    ...
    try {
      ...
      setError(
        err.message || 'An unknown error occurred while fetching fields.'
      );
  ...
  const handleSyncData = useCallback(async () => {
    if (!user) {
      toast.error('Authentication Error', {
        description: 'User context is missing. Cannot sync data.',
      });
      return;
    }
    setIsSyncing(true);
    try {
      ...
      if (success) {
        toast.success('Data Synchronized', {
          description: 'Your offline data has been synced with the server.',
        });
        ...
        toast.error('Sync Failed', {
          description:
            syncError || 'Could not sync data. Some items may have failed.',
          action: details
            ? {
                label: 'View Details',
                onClick: () => console.log('Sync Errors:', details),
              }
            : undefined,
        });
      }
      ...
      toast.error('Sync Error', {
        description: 'A critical error occurred during data synchronization.',
      });
    } finally {
      setIsSyncing(false);
    }
    ...
    <div className={`bg-white shadow rounded-lg p-6 ${className}`}>
      <div className="flex justify-between items-center mb-6">
        ...
        <Button
          ...
          disabled={isSyncing || authLoading || !user}
          variant="outline"
          size="sm"
        >
          {isSyncing ? (
