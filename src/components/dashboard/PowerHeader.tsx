...
export default function PowerHeader({
  ...
  const [currentStatus, setCurrentStatus] = useState(
    synced ? 'All fields synced' : 'Syncing...'
  );
  ...
  const [aiTip, setAiTip] = useState(
    'Rain in 2 days. Sell maize before prices drop.'
  );
      ...
      <div className="p-4 pb-2 relative z-10">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <p
              className="text-sm text-muted-foreground mt-1 animate-fade-in opacity-0"
            ...
            <div className="flex items-center mt-3 gap-3">
              <div
                className="flex flex-col animate-fade-in opacity-0"
                style={{
                  animationDelay: '0.4s',
              ...
              >
                <div className="flex items-center">
                  <span className="text-lg font-bold">
                    ₦{farmValue.value.toLocaleString()}
                  </span>
                  <span
                    className={`ml-1 text-xs flex items-center ${farmValue.change >= 0 ? 'text-green-500' : 'text-red-500'}`}
                  >
                    {farmValue.change >= 0 ? (
            ...
            {isAiTipVisible && (
              <div
                className="mt-3 bg-primary/10 p-2 rounded-lg border border-primary/20 flex items-start gap-2 animate-fade-in opacity-0"
                style={{
                  animationDelay: '0.6s',
                  animationFillMode: 'forwards',
                }}
            ...
            <div
              className="flex items-center mt-3 gap-2 animate-fade-in opacity-0"
              style={{ animationDelay: '0.8s', animationFillMode: 'forwards' }}
            >
              <div className="flex items-center text-sm text-muted-foreground">
                {getWeatherIcon()}
                <span className="ml-1">
                  {location} — {temperature > 0 ? `${temperature}°C` : ''}{' '}
                  {weatherCondition}
                </span>
              </div>
              ...
              <span className="text-xs text-muted-foreground flex items-center ml-auto">
                <Circle
                  className={`h-2 w-2 mr-1 ${isOnlineStatus ? 'text-green-500' : 'text-orange-500'} fill-current`}
                />
                {currentStatus}
              </span>
            </div>
