...
export default function AddFieldForm({
  ...
  const fetchLocationName = trackOperation(
    'fetchLocationName',
    async (lng: number, lat: number) => {
      try {
        console.log(
          `🔍 [AddFieldForm] Reverse geocoding location: ${lng}, ${lat}`
        );

        if (!MAPBOX_ACCESS_TOKEN) {
          console.error(
            '❌ [AddFieldForm] Missing Mapbox access token for reverse geocoding'
          );
        ...
        const response = await fetch(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${lng},${lat}.json?access_token=${MAPBOX_ACCESS_TOKEN}&types=place,locality,neighborhood,address`
        );
          ...
          <div className="grid gap-6 md:gap-8">
            <Card className="overflow-hidden">
              ...
              <CardContent className="pt-4">
                <p className="text-xs text-muted-foreground">
                  {searchedLocation ? (
                    <span>
                      Location: <strong>{searchedLocation}</strong>
                    </span>
                  ) : (
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Basic details about your field
                </CardDescription>
              </CardHeader>
