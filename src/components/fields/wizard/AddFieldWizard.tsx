...
export default function AddFieldWizard({
  ...
  const { logError, logSuccess, trackOperation } =
    useErrorLogging('AddFieldWizard');
  ...
  useEffect(() => {
    const loadFarmContext = async () => {
      try {
        ...
        if (!user?.id) {
          console.log(
            '⚠️ [AddFieldWizard] No user authenticated yet. Proceeding anyway.'
          );
          setIsLoading(false);
          return;
        }
        ...
        // Or use the first available farm
          ...
          console.log(
            '⚠️ [AddFieldWizard] Selected farm not found. Using first farm:',
            farms[0]
          );
        ...
        // If no farms exist, create a default one
          ...
          try {
            ...
            console.error(
              '❌ [AddFieldWizard] Error creating default farm:',
              err
            );
  ...
  const handleSubmit = trackOperation('submitField', async () => {
    try {
        ...
        try {
          ...
          if (existingFarms && existingFarms.length > 0) {
            ...
            if (newFarm) {
              ...
              console.log(
                '✅ [AddFieldWizard] Created emergency farm:',
                newFarm.id
              );
            }
          }
          ...
          console.error(
            '❌ [AddFieldWizard] Error in emergency farm creation:',
            err
          );
      ...
      console.log(
        '💾 [AddFieldWizard] Inserting field data:',
        supabaseFieldData
      );
      ...
      if (userId && targetFarmId && isOnline()) {
        try {
          ...
          if (error) {
            ...
            console.log(
              '✅ [AddFieldWizard] Retry insert succeeded:',
              retryData
            );
          } else {
            fieldResult = data;
            console.log(
              '✅ [AddFieldWizard] Field created successfully:',
              data
            );
          }
        } catch (error) {
          console.error(
            '❌ [AddFieldWizard] Could not create field in database:',
            error
          );
          ...
          const offlineFields = JSON.parse(
            localStorage.getItem('cropgenius_offline_fields') || '[]'
          );
          offlineFields.push(fieldResult);
          localStorage.setItem(
            'cropgenius_offline_fields',
            JSON.stringify(offlineFields)
          );

          toast.warning('Saved locally', {
            description:
              'Your field was saved offline and will sync when possible',
          });
        }
      } else {
      ...
      if (fieldData.crop_type && fieldResult?.id) {
        try {
          ...
          if (userId) {
            ...
            if (cropError) {
              console.warn(
                '⚠️ [AddFieldWizard] Error creating crop record:',
                cropError
              );
              // Store locally as fallback
              const offlineCrops = JSON.parse(
                localStorage.getItem('cropgenius_offline_crops') || '[]'
              );
              offlineCrops.push({ ...cropData, id: uuidv4() });
              localStorage.setItem(
                'cropgenius_offline_crops',
                JSON.stringify(offlineCrops)
              );
            }
          } else {
            // Store locally
      ...
      toast.info('Field saved with limited data', {
        description:
          "Some information couldn't be processed, but your field was saved",
      ...
      <div className="space-y-6">
        ...
        {farmContext && (
          <div className="text-center text-xs text-muted-foreground">
            ...
            <span className="font-medium">{farmContext.name}</span>
          </div>
        )}
          ...
          >
            ...
            {currentStep === 6 && (
              <StepFive
                ...
                onPlantingDateChange={(planting_date) =>
                  updateFieldData({ planting_date })
                }
                onBack={handleBack}
