...
export default function StepOne({
      ...
      try {
        ...
        if (user?.id && farmId) {
          try {
            ...
            if (farmError) {
              console.warn(
                '⚠️ [StepOne] Could not fetch farm data:',
                farmError
              );
            }
            ...
            if (fieldError) {
              console.warn(
                '⚠️ [StepOne] Could not fetch field data:',
                fieldError
              );
            }
            ...
            if (existingNames.length > 0) {
              ...
              existingNames.forEach((name) => {
                ...
                const number =
                  existingNames.filter((n) => n.startsWith(baseName)).length +
                  1;
  ...
  const generateAISuggestion = async () => {
    ...
    try {
      ...
      const fallbackName =
        'AI Generated Field ' + Math.floor(Math.random() * 1000);
