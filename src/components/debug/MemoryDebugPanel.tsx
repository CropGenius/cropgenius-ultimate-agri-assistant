    ...
    <div className="fixed bottom-20 right-4 z-40 w-80 max-h-[500px] overflow-y-auto">
      <Card className="shadow-lg border-gray-200 dark:border-gray-700">
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-xs">Developer tools for memory system</CardDescription>
        </CardHeader>
          ...
          <div className="space-y-1">
            ...
            <div className="grid grid-cols-2 gap-1">
              <Button 
                ...
                onClick={() => simulateMemoryScenarios.newUser().then(refreshMemorySnapshot)}
              >
                New User
              </Button>
              <Button 
                ...
                variant="outline" 
                className="h-7 text-xs w-full"
                onClick={() => simulateMemoryScenarios.returningUser().then(refreshMemorySnapshot)}
              >
                Returning User
              </Button>
              <Button 
                ...
                variant="outline" 
                className="h-7 text-xs w-full"
                onClick={() => simulateMemoryScenarios.powerUser().then(refreshMemorySnapshot)}
              >
                Power User
              </Button>
              <Button 
                ...
                variant="outline" 
                className="h-7 text-xs w-full"
                onClick={() => simulateMemoryScenarios.referredUser().then(refreshMemorySnapshot)}
              >
                Referred User
              </Button>
              <Button 
                ...
                variant="outline" 
                className="h-7 text-xs w-full"
                onClick={() => simulateMemoryScenarios.offlineUser().then(refreshMemorySnapshot)}
              >
                Offline User
              </Button>
            </div>
